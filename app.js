...
function applyZoom() {
  ...
  /* Dots scale at 30% of the zoom rate:
     visual size = 1 + (zoom.scale - 1) * 0.3
     transform   = visualSize / zoom.scale  (to cancel parent scale then apply desired size) */
  const visualScale = 1 + (zoom.scale - 1) * 0.3;
...
function openFeature(i) {
  ...
  if (window.matchMedia('(orientation: portrait)').matches) {
    /* Portrait — slide-up bottom sheet, always at screen bottom regardless of zoom */
