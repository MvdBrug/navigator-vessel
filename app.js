...
function applyZoom() {
  ...
  /* Dots scale at 20% of the zoom rate:
     visual size = 1 + (zoom.scale - 1) * 0.2
     transform   = visualSize / zoom.scale  (to cancel parent scale then apply desired size) */
  const visualScale = 1 + (zoom.scale - 1) * 0.2;
  const dotT  = visualScale / zoom.scale;          /* normal dot */
  const dotTA = visualScale * 1.3 / zoom.scale;    /* active dot — slightly larger */
...
function openFeature(i) {
  ...
  if (window.matchMedia('(orientation: portrait)').matches) {
    /* Portrait — clear any inline position left/top set by a previous landscape open,
       so the CSS bottom:0 rule is not overridden by inline styles */
    popup.style.left = '';
    popup.style.top  = '';
...
/* Orientation change — use orientationchange (more reliable on iOS than resize)
   plus a 300ms delay so the viewport fully settles before repositioning */
...
/* Keep resize as fallback for desktop / non-iOS devices */
